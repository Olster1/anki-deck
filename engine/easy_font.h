...
static void easyFont_createSDFFont(char *fileName) {
    ...
    char *fontName = getFileLastPortionWithArena(fileName, &globalPerFrameArena);
    ...
    char *outputFileName = concatInArena(globalExeBasePath, "./fontAtlas", &globalPerFrameArena);

    // NOTE(ollie): Try find the smallest size to fint everyone in
    do {
        dimensions = easyAtlas_drawAtlas(outputFileName, &globalPerFrameArena, &atlasElms, false, "null", 5, rectX, rectY, EASY_ATLAS_FONT_ATLAS);
    ...
    easyAtlas_drawAtlas(outputFileName, &globalPerFrameArena, &atlasElms, true, "_mono-font", 5, rectX, rectY, EASY_ATLAS_FONT_ATLAS);
...
static EasyFont_Font easyFont_loadFontAtlas(char *fileName) {

    EasyFont_Font font = {};

    char buffer0[4096] = {};
    snprintf(buffer0, arrayCount(buffer0), "%s.txt", fileName);
    
    char buffer1[4096] = {};
    snprintf(buffer1, arrayCount(buffer1), "%s.png", fileName);
